...
from service.time_converter import time_elapsed_in_hours

def calculate_time_elapsed(readings):
    minimum_time = min(map(lambda x:x.time, readings))
    maximum_time = max(map(lambda x:x.time, readings))
    return time_elapsed_in_hours(minimum_time, maximum_time)

class PricePlanService:
    def __init__(self, reading_repository) -> None:
        self.electricity_reading_service = ElectricityReadingService(repository=reading_repository)
        pass
    ...
    def get_list_of_spend_against_each_price_plan_for(self, smart_meter_id, limit=None):
        ...
        average = self.calculate_average_reading(readings=readings)
        time_elapsed = calculate_time_elapsed(readings=readings) 
