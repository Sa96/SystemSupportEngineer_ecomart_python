from controller.electricity_readings_controller import service as electricity_reading_service
from domain.price_plan import PricePlan
from generator.electricity_readings_generator import generate_electricity_readings
...
def populate_random_electricity_readings():
    for index in range(NUM_METERS):
        electricity_reading_service.store_reading({
            "smartMeterID":f"smart-meter-{index}", "ElectricityReadings":generate_electricity_readings(NUM_READINGS_AGAINST_METER)
        })
