...
from src.controller.electricity_readings_controller import repository as reading_repository
...
class TestPricePlanComparatorController(unittest.TestCase):
    ...
    def tearDown(self) -> None:
        ...
        reading_repository.clear()

    def test_get_cost_against_all_price_plans(self):
        result = self.client.get('/price-plans/compare-all/smart-meter-1')
        self.assertEqual(result.status_code,200)
        self.assertEqual(result.get_json()['PricePlanId'], 'price-plan-1')
        self.assertEqual(len(result.get_json()['pricePlanComparisons']), 3)
