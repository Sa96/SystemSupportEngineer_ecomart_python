...
class TestElectricityReadingRepository(unittest.TestCase):
    def setUp(self) -> None:
        self.electricity_reading_repository = ElectricityReadingRepository()
        self.electricity_reading_1 = ElectricityReading({'time':1507375234, 'reading':0.5}) 
        self.electricity_reading_2 = ElectricityReading({'time':1510053634, 'reading':0.75})
        self.electricity_reading_repository.store(
            'smart-meter-0',
            [self.electricity_reading_1, self.electricity_reading_2]
        )
    ...
    def test_add_usage_data_against_smart_meter_id_if_it_already_exists(self):
        self.electricity_reading_repository.store('smart-meter-0', [ElectricityReading({'time': 1510572000, 'reading': 0.32})])
        readings = self.electricity_reading_repository.find('smart-meter-0')
